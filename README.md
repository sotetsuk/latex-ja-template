...
- ✅ 軽量な TeX Live 構成（`scheme-infraonly` + 必要パッケージのみを明示インストールし、従来の約 1.5GB から縮小。実サイズは CI のビルドログで確認可能）
...
- **不足パッケージのエラー**: 最小構成（必要パッケージのみ明示インストール）のため、未収録のパッケージがある場合があります。`sudo tlmgr install <パッケージ名>` で追加できます。
