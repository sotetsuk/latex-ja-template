...
% minted/fvextra は verbatim 内の " 等に \textquotedbl を用いるため、
% 8bit エンジン(upLaTeX)では T1 エンコーディングが必要（OT1 には無い）。
...
\section{コード（minted）}
minted はシンタックスハイライトに対応します（upLaTeX$\to$dvipdfmx の経路でも、
TeX Live 同梱の minted v3 / latexminted により追加設定なしで動作）。
インラインは \mintinline{python}{lambda x: x+1}。
